...
import "./ActionTaken.scss";
import { TextInput } from "@/components";
import { Formik } from "formik";
import * as Yup from "yup";
import { useDispatch } from "react-redux";
import { addActionTaken, getActionTaken } from "@/store/globalSlice";

const ActionTaken = ({ onHide, id }) => {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const res = await dispatch(getActionTaken(id));
        if (res?.status) {
  ...
  const handleSave = async (values, { resetForm }) => {
    ...
    const res = await dispatch(addActionTaken(payload));
    if (res?.status === 200) {
      resetForm();
      onHide();
    }
  ...
  return (
    <div id="actiontakenconfirmation-container">
      <Modal
        title="Action Taken"
        width="600px"
        onHide={onHide}
